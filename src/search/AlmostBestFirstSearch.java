...
public class AlmostBestFirstSearch extends Search
{
	...
	public AlmostBestFirstSearch(State s)
    {
		this(s, new HValueComparator());
		bestHValue = s.getHValue();
		bestState = s;
	}

	public AlmostBestFirstSearch(State s, Comparator c)
    {
		super(s);
		setComparator(c);
	...
	public State search() {
		...
		while (!open.isEmpty())
		{
			...
			if (needToVisit(s)) {
				...
				else if (s.getHValue().compareTo(bestHValue) < 0) {
					return s;
				}
				else {
					updateOpen(s);
			...
			javaff.JavaFF.infoOutput.println("ABFS :" + nodeCount);
