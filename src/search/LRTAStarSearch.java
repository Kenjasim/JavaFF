...
public class LRTAStarSearch extends Search
{
	...
	public State search()
	{
		...
		while (!open.isEmpty()  && depth < maxDepth) // whilst still states to consider
		{
			...
			javaff.JavaFF.infoOutput.println("------------------------------------------");
			javaff.JavaFF.infoOutput.println("Current State: " + s);
			javaff.JavaFF.infoOutput.println("Current HValue: " + currentHValue);
			...
			while (succItr.hasNext())
			{
				...
				// javaff.JavaFF.infoOutput.println(succ + " - " + succ.getHValue());
				if (needToVisit(succ))
				{
					// javaff.JavaFF.infoOutput.println(succ + " - hard - " + succ.getHValue());
