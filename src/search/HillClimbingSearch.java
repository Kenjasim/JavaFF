...
public class HillClimbingSearch extends Search
{
	...
	public HillClimbingSearch(State s, int randValue)
	{
	...
	public State search() {
		...
		{	
			...
			Set<State> bestsucessor = new HashSet<State>();	//Empty set of best sucessors
			...
			while (succItr.hasNext()) {
				...
				if (needToVisit(succ)) {
					if (succ.goalReached()) { // if we've found a goal state - return it as the solution
						...
						bestsucessor.add(succ); //add to best successor 
					}else if (succ.getHValue() == bestHValue) {
						bestsucessor.add(succ); //add to best successor 
						open = new LinkedList(); // clear the open list
			...
			if (bestsucessor.isEmpty())
			{
				s = null;
			}else
			{
				Iterator itr = bestsucessor.iterator();
				int i = 0;
				while(itr.hasNext())
				{
					if (i == randValue)
					{
						return((State) itr.next());
					}
				}
				return null;
